...
let i_secasgn_lv (_loc : EcLocation.t) (_env : EcEnv.env) lv e =
  match lv with
  | Lval lv -> i_secasgn (lv, e)
  | LvMap ((op,tys), x, ei, ty) ->
    let op = e_op op tys (toarrow [ty; ei.e_ty; e.e_ty] ty) in
    i_secasgn (LvVar (x,ty), e_app op [e_var x ty; ei; e] ty)
...
let i_rnd_lv loc env lv e =
  ...
  | LvMap ((op,tys), x, ei, ty) ->
    let op = e_op op tys (toarrow [ty; ei.e_ty; e.e_ty] ty) in
    i_rnd (LvVar (x,ty), e_app op [e_var x ty; ei; e] ty)
(*   | LvMap _ -> tyerror loc env LvMapOnNonAssign *)

let i_secrnd_lv loc env lv e =
  match lv with
  | Lval lv -> i_secrnd (lv, e)
  | LvMap ((op,tys), x, ei, ty) ->
    let op = e_op op tys (toarrow [ty; ei.e_ty; e.e_ty] ty) in
    i_secrnd (LvVar (x,ty), e_app op [e_var x ty; ei; e] ty)
(*   | LvMap _ -> tyerror loc env LvMapOnNonAssign *)
...
(*       [ i_secrnd_lv i.pl_loc env lvalue rvalue ] *)
